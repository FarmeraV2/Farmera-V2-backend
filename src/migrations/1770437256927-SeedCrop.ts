...
export class SeedCrop1770437256927 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await this.runSqlFiles(queryRunner, "up");
        await queryRunner.query(`
            SELECT setval(
                pg_get_serial_sequence('crop', 'id'),
                (SELECT MAX(id) FROM crop)
            );
        `);
    }
    ...
    private async runSqlFiles(queryRunner: QueryRunner, dir: string) {
        const folderPath = join(__dirname, "/../database/sql/1770437256927-SeedCrop", dir);

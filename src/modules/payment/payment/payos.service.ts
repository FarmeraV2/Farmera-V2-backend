...
export class PayosService {
    ...
    async createPayOSOrder(
        ...
        this.logger.debug('PayOS Request URL:', this.payOSCreateOrderUrl);
        this.logger.debug('PayOS Headers:', headers);
        this.logger.debug('PayOS Body:', body);
        ...
        try {
            const response = await firstValueFrom(
                this.httpService.post<ResponseOrderPayOSDto>(
                    ...
                    map(axiosResponse => {
                        this.logger.debug('PayOS Raw Response:', axiosResponse.data);
                        const responseData = axiosResponse.data;
                        
                        if (responseData.code === '00') {
                            return responseData;
                        } else {
                            this.logger.error(`PayOS Error Code: ${responseData.code}, Message: ${responseData.desc}`);
                            throw new Error(`PayOS error [${responseData.code}]: ${responseData.desc}`);
                        }
                    }),
                    catchError(error => {
                        this.logger.error('PayOS HTTP Error:', error.response?.data || error.message);
                        throw error;
                    })
                )
            );
            ...
            this.logger.error('PayOS Request Failed:', {
